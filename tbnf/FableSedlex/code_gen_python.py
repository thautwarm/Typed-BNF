...
def codegen_python(cu: compiled_unit) -> Doc:
    ...
    def push_later_toplevel(doc_1: Doc, cu=cu) -> None:
        nonlocal later_toplevels
        later_toplevels = of_array_with_tail([doc_1, empty_2], later_toplevels)
    
    st_func_name : Callable[[int], str] = lambda i, cu=cu: to_text(printf("_sedlex_st_%d"))(i)
    def _cg_decision_func(tree: Automata_decision_tree, cu=cu) -> Doc:
    ...
    def cg_decision_func(tree_1: Automata_decision_tree, cu=cu) -> str:
    ...
    def _cg_state_func(lang: lang_3, cu=cu) -> Doc:
        if lang.tag == 3:
            ...
            def arrow_53(lang=lang) -> Doc:
                lst_1 : FSharpList[Doc] = map_1(lambda s_2: word(s_2), names_1)
                return seplist(word(", "), lst_1)
            
            push_later_toplevel(Doc_op_Addition_Z7CFFAC00(Doc_op_Addition_Z7CFFAC00(word(func_table), word("=")), bracket(arrow_53())))
    ...
    token_ids : FSharpList[Doc] = of_array(map_2(lambda _arg1, cu=cu: pretty(_arg1.fields[0]) if (_arg1.tag == 1) else (word("None")), pattern_input[0], None))
    ...
    return vsep(append(singleton(vsep(of_array([word("from fable_sedlex.sedlex import *"), word("import typing"), word("import typing_extensions"), word("import dataclasses")]))), append(toplevels, append(singleton(middle_toplevels), later_toplevels))))
