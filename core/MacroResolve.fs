...
let rec toPositionIndependent (term: symbol) =
    ...
    | Nonterm _
    | Term _ -> term
    | Macrocall(name, syms, _) -> Macrocall(name, List.map toPositionIndependent syms, position.Fake)
...
let resolve_macro
    ...
    for each in stmts do
        match each with
        | Defmacro decl -> macro_defs <- Map.add decl.lhs decl macro_defs
        | Defrule decl ->
            let scope = Map.empty
            // printfn "adding %s" decl.lhs
            stmts_to_solve <- (decl, scope) :: stmts_to_solve
        | _ -> fixed_stmts <- each :: fixed_stmts
    // printfn "solving..."
    // stmts_to_solve
    ...
    let fixedPoint =
        fun () ->
            ...
            let rec solve_specialization
                (
                    decl:
                        {| define: list<position * production>
                           lhs: string
                           pos: position |},
                    ...
                    | None ->
                        ...
                        | Some macro_def ->
                            ...
                            if List.length parameters <> List.length args then
                                ...
                                let msg =
                                    $"macro {n} expects {parameters.Length} argument(s): ({str_parameters}); got {args.Length}"

                                raise <| MacroResolveError msg

                            let scope' = List.map2 (fun k v -> k, v) parameters args |> Map.ofList
