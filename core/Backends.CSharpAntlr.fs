...
let codegen (analyzer: Analyzer) (cg_options: CodeGenOptions) (langName: string) (stmts: definition array) =
    ...
    let abandoned_names = Set.ofArray <| Array.append [| "result" |] CSharpKeywords
    ...
    let rec _cg_type (t: monot) =
        ...
        // it is impossible to alias System.Array, so we need to handle it separately
        | monot.TApp(f, args) ->
            let f = _cg_type f

            match args with
            | [ arg ] when f = "System.Array" -> _cg_type arg + "[]"
            | _ ->
                args
                |> List.map _cg_type
                |> String.concat ", "
        ...
        <| fun () ->
            ...
            let rec cg_expr (scope: list<string * string>) (curr_expr: expr) : block<Doc> =
                ...
                cg {
                    ...
                    | node.EFun(args, body) ->
                        let localScope = [ for arg, t in args -> arg, mangle csharpIdentDescr arg, t ]
                        let scope' = List.map (fun (user, mangled, _) -> user, mangled) localScope @ scope
                        let returned, code = runCG <| cg_expr scope' body

                        return
                            defineCSharpFunc (
                                List.map (fun (a, b) -> word (cg_type b + " " + a))
                                <| List.map (fun (_, mangled, typ) -> mangled, typ) localScope
                            )
                    ...
                    | node.ELet(n, value, body) ->
                        ...
                        yield word (cg_type value.t) + word m_name + word "=" + value' * word ";"
    ...
    let rec cg_ruledef (lhs: string) (define: list<position * production>) =
        ...
        |> fun body ->
            vsep
                [ word ntname + word "returns" + bracket (word (cg_type t) + word resultName)
                  body >>> 4
                  word ";" ]
            ...
            [
              ...
              for (typename, cases) in adtCases do
                  ...
                  for (ctorName, fields) in Map.toArray cases do // TODO: give a proper error when cases are empty
                      ...
                      let func_params =
                          parens (seplist (word ",") [ for (fname, t) in fields -> t + fname ])
              ...
              for (typename, shape) in analyzer.Sigma.GetRecordTypes() do
                  ...
                  yield
                      word $"public partial record {typename'}"
                      * word tparams
                      * func_params
                      * word ";"
                    ...
                    vsep
                        [ yield word $"grammar {langName};"
                          ...
                          for (tparams, function_name, ctor_name, fields, ret_t) in docCtorWrapFuncs do
                              let func_params =
                                  parens (seplist (word ",") (List.map (fun (fname, t) -> t + fname) fields))
                                            ...
                                            if tparams = "" then
                                                ...
                                                vsep
                                                    [ word "return"
                                                      + parens (ret_t)
                                                      + word "new"
                                                      + word ctor_name * word tparams * args * word ";" ]
                                                >>> 4
                                        yield word "}" ]
                          ...
                          yield
                              word (
                                  $"start returns [{cg_type start_t} result]: v={start_mangled} EOF"
                                  + "{ $result = _localctx.v.result; };"
                              )
                          yield file_grammar
                          yield! lexerDefs ]
