...
type Sigma(UM: Unification.Manager, errorTrace: ErrorTrace) =
    ...
    let addCase (typename: typename) ctorName t =
        ...
        | Some { parameters = _ :: _ as parameters } ->
            raise
            <| InvalidConstructorDefinination(ctorName, CauseGenericADTType(typename, parameters))
        | None ->
        ...
        let mutable adtCases' =
            ...
            |> List.replaceWith typename (function
                ...
                | Some cases -> Map.add ctorName t cases)
            ...
            if external then
                ...
                let t =
                    if List.isEmpty parameters then
                        ...
                        let tapp =
                            TFun(fields, TApp(TConst typename, List.map TVar parameters))
                            |> processPolyType parameters

                        let fields =
                            match tapp with
                            | TFun(fields, _) -> fields
    ...
    let lookupField t fieldname (tyref: monot) =
        ...
        | Some shape ->
            ...
            | Some ft ->
                ...
                UM.Instantiate(gen_spec) |> snd |> (fun spec -> UM.Unify(inst_target, spec))
    ...
    let registerCtorGVar varname (t: monot) =
        if Map.containsKey varname global_variables then
            ...
            match t with
            | TFun(args, TConst typename) -> addCase typename varname args
    ...
    member __.SetCurrentPos(pos: position) = errorTrace.currentPos <- pos
    member __.CurrentPos = errorTrace.currentPos

    member __.SetCurrentDefinition(def: definition) =
        errorTrace.whichDef <- def
        errorTrace.branch <- 0

    member __.SetCurrentDefinitionBranch(branch: int) = errorTrace.branch <- branch
    ...
    member __.RegisterType external hasFields typename parameters fields =
        defineShape external hasFields typename parameters fields
...
let build_analyzer (stmts: definition array) =
    let errorTrace =
        { ErrorTrace.whichDef = stmts.[0]
          ErrorTrace.exprStack = []
          currentPos = position.Fake
          branch = 0 }
    ...
    let pre_process (stmt: definition) =
        ...
        | definition.Declvar decl -> Sigma.KindCheck decl.t |> Sigma.RegisterExtGVar decl.ident

        | definition.Declctor decl -> Sigma.KindCheck decl.t |> Sigma.RegisterCtorGVar decl.ident
    ...
    let rec check_lexerule (x: lexerule) =
        ...
        | LStr _ -> ()
        | LSeq seq
        | LOr seq -> List.iter check_lexerule seq
        | LGroup l
        | LOptional l
        | LStar l
        | LPlus l -> check_lexerule l
    ...
    let main () =
            ...
            Array.append
                [| definition.Defmacro
                       {| lhs = "tbnf-alternative2"
                          ...
                          define =
                           [ position.Fake,
                             { symbols = [ symbol.Nonterm("a") ]
                               action =
                                 { node = ESlot 1
                                   t = TConst_int
                                   pos = position.Fake } }
                             position.Fake,
                             { symbols = [ symbol.Nonterm("b") ]
                               action =
                                 { node = ESlot 1
                                   t = TConst_int
                                   pos = position.Fake } } ] |} |]
        ...
        for stmt in stmts do
            ...
            match stmt with
            | Defrule decl -> check_productions (decl.lhs, decl.define)
            | Deflexer decl -> check_lexerule decl.define
