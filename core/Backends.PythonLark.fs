...
let codegen (analyzer: Analyzer) (cg_options: CodeGenOptions) (langName: string) (stmts: definition array) =
    ...
    let pythonIdentifierDescr =
        IdentifierDescriptor
            .Create(
                ...
                (fun i c -> if isDigit c then $"_{i * int c + 7}_" else $"_{int c}_")
            )
            .WithNameEnv
    ...
    let _name_of_named_term (name: string) = mangle larkLexerIdentifierDescr name

    let _name_of_nonterm (name: string) = mangle larkParserIdentifierDescr name
    ...
    let cg_symbol (x: symbol) =
        ...
        | None ->
            ...
            | Term(define, is_literal) ->
                ...
                if is_literal then _name_of_literal_term define
        ...
        <| fun () ->
            cg {
                ...
                | node.EFun(args, body) ->
                    ...
                    yield
                        definePyFunc (word func_name) (List.map word args)
                        <| vsep [ vsep code; word "return" + returned ]
                ...
                | node.ESlot i -> return word ($"{TREE_NAME}[{i - 1}]")
                | node.ETuple elts ->
                    let! elts' =
                        cg {
                            for elt in elts do
    ...
    let rec cg_stmt (stmt: definition) =
        ...
        | definition.Declvar decl ->
            ...
            empty
        | definition.Declctor decl -> empty
    ...
    <| fun () ->
        ...
        |> fun file_grammar ->
                ...
                vsep
                    [ yield word $"from __future__ import annotations as __01asda1ha"
                      ...
                      for (typename, cases) in adtCases do
                          ...
                          yield
                              vsep
                                  [ match docCtorNames with
                                    | [ t ] -> yield word typename' + word "=" + t
                                    | _ ->
                                        yield
                                            word typename'
                                            + word "="
                                            + word $"{modulevar_typing}.Union["
                                              * seplist (word ",") docCtorNames
                                              * word "]" ]
                              >>> 4
                          ...
                          yield
                              vsep [ word typename' + word "=" + parens (seplist (word ",") docCtorNames) ]
                              >>> 4
            ...
            let lexerInfo = (peof, Tokenize -1) :: lexerInfo |> List.rev |> Array.ofList

            let file_grammar: Filename (* string *) * Doc =
                (langName + ".lark",
                 vsep
                     [ file_grammar
                       word "%declare" + seplist (word " ") (List.map word tokenNames) ])
                ...
                vsep
                    [ yield word "from __future__ import annotations"
                      ...
                      yield
                          word var_tokenmaps
                          + word "="
                          + bracket (seplist (word ", ") (List.map (escapeString >> word) tokenNames))
                      yield
                          word var_tokenreprs
                          + word "="
                          + bracket (seplist (word ", ") (List.map (escapeString >> word) tokenReprs))
                      yield empty
                      yield
                          definePyFunc
                              (word var_construct_token)
                              [ word "token_id"
                                word "lexeme"
                                word "line"
                                word "col"
                          ...
                          <| vsep
                              [ word $"if token_id == -1: return {classvar_LarkToken}(\"EOF\", \"\")"
                                word
                                    $"return {classvar_LarkToken}({var_tokenmaps}[token_id], lexeme, offset, line, col, None, None, span + offset)" ]
                      yield empty
                      yield
                          definePyFunc (word var_iseof) [ word "token" ]
                      ...
                      yield
                          word
                              $"with (__import__('pathlib').Path(__file__).parent /'{langName}.lark').open(encoding='utf8') as __0123fx9:"
                      yield word export_Grammar + word "= __0123fx9.read()" >>> 4
                      yield empty
                      yield
                          word export_Parser
                          + word "="
                          + word classvar_LarkBuilder
                            * parens (
                                seplist
                                    (word ", ")
                                    [ word export_Grammar
