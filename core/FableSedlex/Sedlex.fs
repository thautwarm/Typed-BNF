...
module Automata =
    ...
    let compile_re re =
        let final = new_node () in (re final, final)
    ...
    let rec add_node state node =
        if List.exists ((==) node) state then
            state
        else
            add_nodes (node :: state) node.eps

    and add_nodes state nodes = List.fold add_node state nodes
    ...
    let transition (state: state) =
        ...
        let split (all, t) (c0, n0) =
            ...
            Cset.union all c0, List.filter (fun (c, _) -> not (Cset.is_empty c)) t in
    ...
    let old_decision_table l =
        let rec aux m accu =
            function
            | ((a, b, i) as x) :: rem when b < limit && i < 255 -> aux (min a m) (x :: accu) rem
            | rem -> m, accu, rem in
        ...
        match table with
        | [] -> decision l
        | [ (min, max, i) ] -> Lte(min - 1, Return(-1), (Lte(max, Return i, decision rest)))
        | (_, max, _) :: _ ->
    ...
    let segments_of_partition p =
        let seg = ref [] in
        Array.iteri (fun i c -> List.iter (fun (a, b) -> seg := (a, b, i) :: !seg) c) p
    ...
    let rec char_pair_op func (name: string) p0 p1 = (* Construct something like Sub(a,b) *)
        ...
        | Some r -> r
        | None -> invalidOp $"the {name} operator can only applied to single-character length regexps"
...
let public_next_int lexbuf : int =
    ...
    if (lexbuf.pos = lexbuf.len) && lexbuf.finished then
        -1
    else
...
let lexeme_char lexbuf pos = lexbuf.src[lexbuf.start_pos + pos]
...
module Utf8 =
    ...
    let lexeme_char lexbuf pos = lexeme_char lexbuf pos
    ...
    let rec evaluate_decision_func tree =
        ...
        | None ->
            let f =
                ...
                | Return i -> fun _ -> i
                | Table(offset, t) -> fun c -> t[c - offset] - 1
...
let pseq xs =
    let _pseq xs =
        ...
        | p :: p1 -> List.fold (fun r p -> seq r p) p p1
...
let prep p0 i1 i2 =
    if 0 <= i1 && i1 <= i2 then
        ...
        invalidOp ($"repeat operator requires 0 <= {i1} <= {i2}")
...
let pcompl arg =
    ...
    | Some r -> r
    | None -> invalidOp "the Compl operator can only applied to a single-character length regexp"

let psub arg = char_pair_op subtract "sub" arg

let pintersct arg =
    char_pair_op intersection "intersect" arg
...
let pinterval i_start i_end = chars (Cset.interval i_start i_end)

let regexp_for_char c = chars (Cset.singleton (int c))
...
let regexp_for_string s =
    let rec aux n =
        if n = String.length s then
            ...
            seq (regexp_for_char s.[n]) (aux (Cset.succ n)) in
