...
from fable_modules.fable_library.util import (compare_primitives, partial_apply, string_hash, get_enumerator, ignore)
...
class Sigma:
    def __init__(self, UM: Manager=None) -> None:
        ...
        self.shapes = empty(ObjectExpr48())
        class ObjectExpr49:
            @property
            def Compare(self) -> Any:
                return lambda x_1, y_1: compare_primitives(x_1, y_1)
            
        self.kinds = empty(ObjectExpr49())
        class ObjectExpr50:
            @property
            def Compare(self) -> Any:
                return lambda x_2, y_2: compare_primitives(x_2, y_2)
            
        self.global_variables = empty(ObjectExpr50())
    

Sigma_reflection = expr_51
...
def Sigma__checkKind__Z25145215(this: Sigma, t: monot) -> None:
    if t.tag == 2:
        ...
        def arrow_52(t_1: monot, this=this, t=t) -> None:
            Sigma__checkKind__Z25145215(this, t_1)
        
        monot__apply_to_children_z6a62bcbf(t, arrow_52)
...
def Sigma__defineShape(this: Sigma, typename: str, parameters: FSharpList[str], fields: FSharpList[Tuple[str, monot]]) -> None:
    if contains_key(typename, this.shapes):
        ...
        def arrow_54(this=this, typename=typename, parameters=parameters, fields=fields) -> Any:
            ...
            return add(typename, Shape(parameters, of_seq(fields, ObjectExpr53())), table)
        
        this.shapes = arrow_54()
...
def build_analyzer(stmts: List[definition]) -> Analyzer:
    ...
    Omega : Any = empty(ObjectExpr56())
    class ObjectExpr57:
        @property
        def Compare(self) -> Any:
            return lambda x_1, y_1: compare_primitives(x_1, y_1)
        
    LiteralTokens : Any = empty_1(ObjectExpr57())
    class ObjectExpr58:
        @property
        def Compare(self) -> Any:
            return lambda x_2, y_2: compare_primitives(x_2, y_2)
        
    ReferencedNamedTokens : Any = empty_1(ObjectExpr58())
    ...
    IgnoreSet : Any = empty_1(ObjectExpr59())
    def infer_e(s_gamma: Any, S: FSharpList[monot], e: expr, stmts=stmts) -> expr:
        ...
        if match_value.tag == 6:
            ...
            return expr(node_5(3, value_4, fieldname_1), e.pos, Sigma__LookupField(Sigma_1, value_4.t, fieldname_1))
        
        elif match_value.tag == 5:
            ann_args : FSharpList[Tuple[str, monot]] = match_value.fields[0]
            body_3 : expr = infer_e(fold(lambda state, tupled_arg_1, s_gamma=s_gamma, S=S, e=e: add(tupled_arg_1[0], polyt(1, Sigma__KindCheck_Z25145215(Sigma_1, tupled_arg_1[1])), state), s_gamma, ann_args), S_1, match_value.fields[1])
    ...
    def check_lexerule(x_9_mut: lexerule, stmts=stmts) -> None:
        while True:
            ...
            if x_9.tag == 10:
                ...
                if contains(x_9.fields[0], TokenFragments, ObjectExpr60()):
    ...
    for idx in range(0, (len(stmts) - 1) + 1, 1):
        ...
        if pattern_matching_result_1 == 0:
            ...
            if stmt.tag == 1:
                ...
                if contains(stmt.fields[0]["lhs"], TokenFragments, ObjectExpr61()):
        ...
        if stmt_2.tag == 1:
            ...
            with get_enumerator(tupled_arg_2[1]) as enumerator_2:
                while enumerator_2.System_Collections_IEnumerator_MoveNext():
                    ...
                    def infer_p(s: symbol, stmts=stmts) -> monot:
                        ...
                        if s.tag == 0:
                            ...
                            if s.fields[0]["is_literal"]:
                                ...
                                if not contains(n, TokenFragments, ObjectExpr62()):
                    ...
                    action : expr = infer_e(empty(ObjectExpr64()), S_2, production.action)
