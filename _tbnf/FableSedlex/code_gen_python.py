...
def codegen_python(import_head: str, cu: compiled_unit) -> Doc:
    ...
    decision_funcs : Any = empty(ObjectExpr48())
    tbl_cnt : int = 0
    dt_cnt : int = 0
    rnd_cnt : int = 0
    def new_rnd_name(import_head=import_head, cu=cu) -> str:
        nonlocal rnd_cnt
    ...
    def push_later_toplevel(doc_1: Doc, import_head=import_head, cu=cu) -> None:
    ...
    def _cg_decision_func(tree: Automata_decision_tree, import_head=import_head, cu=cu) -> Doc:
    ...
    def cg_decision_func(tree_1: Automata_decision_tree, import_head=import_head, cu=cu) -> str:
    ...
    def _cg_state_func(lang: lang_3, import_head=import_head, cu=cu) -> Doc:
        if lang.tag == 3:
            ...
            def arrow_58(lang=lang) -> Doc:
                lst_1 : FSharpList[Doc] = map_1(lambda s_2: word(s_2), names_1)
                return seplist(word(", "), lst_1)
            
            push_later_toplevel(Doc_op_Addition_Z7CFFAC00(Doc_op_Addition_Z7CFFAC00(word(func_table), word("=")), bracket(arrow_58())))
