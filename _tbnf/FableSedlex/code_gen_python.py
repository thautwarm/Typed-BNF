...
def codegen_python(import_head: str, cu: compiled_unit) -> Doc:
    ...
    decision_funcs : Any = empty(ObjectExpr50())
    tbl_cnt : int = 0
    dt_cnt : int = 0
    rnd_cnt : int = 0
    ...
    def _cg_state_func(lang: lang_3, import_head=import_head, cu=cu) -> Doc:
        if lang.tag == 3:
            ...
            def arrow_52(lang=lang) -> Doc:
                lst_1 : FSharpList[Doc] = map_1(lambda s_2: word(s_2), names_1)
                return seplist(word(", "), lst_1)
            
            push_later_toplevel(Doc_op_Addition_Z7CFFAC00(Doc_op_Addition_Z7CFFAC00(word(func_table), word("=")), bracket(arrow_52())))
