...
from fable_modules.fable_library.list import (empty, FSharpList, map as map_1, of_array as of_array_1, is_empty, head, tail, append, cons, to_array, fold, map_indexed, of_array_with_tail, reverse, singleton as singleton_1)
...
from .grammar import (symbol, position, node, monot, expr as expr_1, lexerule, production as production_1, definition as definition_1)
from .utils import (IdHelper_newIdEnv, IdHelper_idEnv, NameMangling_IdentifierDescriptor__WithNameEnv_Z4088684A, NameMangling_nameEnv, NameMangling_IdentifierDescriptor_Create_Z48C5CCEF, is_lower, is_unicode, is_upper, is_digit, NameMangling_IdentifierDescriptor, NameMangling_maskChar, NameMangling_mangle, DocBuilder_Builder__Run_72B9C953, DocBuilder_Builder__Delay_Z6CB7AF07, DocBuilder_Builder__Return_1505, DocBuilder_cg, DocBuilder_Builder__Bind_Z300482AD, DocBuilder_block_1, DocBuilder_runCG, DocBuilder_Builder__Combine_Z44B4C520, DocBuilder_Builder__Yield_417FD60, DocBuilder_Builder__ReturnFrom_72B9C953, DocBuilder_Builder__For_4C40856C)
...
def codegen(analyzer: Analyzer, _arg1: CodeGenOptions, stmts: List[definition_1]) -> List[Tuple[str, Doc]]:
    ...
    def arrow_100(i: int, c: str, analyzer=analyzer, _arg1=_arg1, stmts=stmts) -> bool:
    ...
    python_identifier_descr : NameMangling_IdentifierDescriptor = NameMangling_IdentifierDescriptor__WithNameEnv_Z4088684A(NameMangling_IdentifierDescriptor_Create_Z48C5CCEF(arrow_100, lambda i_1, c_1, analyzer=analyzer, _arg1=_arg1, stmts=stmts: to_text(interpolate("_X%P()_", [i_1])) if (is_digit(c_1)) else (to_text(interpolate("_%P()_", [ord(c_1)])))), NameMangling_nameEnv(of_list(map_1(lambda tuple, analyzer=analyzer, _arg1=_arg1, stmts=stmts: tuple[1], global_scope), ObjectExpr99())))
    class ObjectExpr101:
        @property
        def Compare(self) -> Any:
            return lambda x_5, y_5: compare_primitives(x_5, y_5)
        
    lark_lexer_identifier_descr : NameMangling_IdentifierDescriptor = NameMangling_IdentifierDescriptor__WithNameEnv_Z4088684A(NameMangling_IdentifierDescriptor_Create_Z48C5CCEF(lambda i_2, c_2, analyzer=analyzer, _arg1=_arg1, stmts=stmts: is_upper(c_2) if (i_2 == 0) else (True if (True if (is_upper(c_2)) else (c_2 == "_")) else (is_digit(c_2))), lambda _arg1_1, c_3, analyzer=analyzer, _arg1=_arg1, stmts=stmts: c_3.upper() if (is_lower(c_3)) else (("_" + NameMangling_maskChar(ord("A"), ord("Z"), ord(c_3))) + "_")), NameMangling_nameEnv(of_array(["UNKNOWN"], ObjectExpr101())))
    class ObjectExpr102:
        @property
        def Compare(self) -> Any:
    ...
    def cg_symbol(x_7: symbol, analyzer=analyzer, _arg1=_arg1, stmts=stmts) -> str:
        nonlocal symmap
    ...
    def try_lookup(key_mut: a_, x_8_mut: FSharpList[Tuple[a_, b_]], analyzer=analyzer, _arg1=_arg1, stmts=stmts) -> Optional[b_]:
        while True:
            (key, x_8) = (key_mut, x_8_mut)
            if not is_empty(x_8):
                if equals(head(x_8)[0], key):
                    return some(head(x_8)[1])
                
                elif not is_empty(x_8):
                    key_mut = key
                    x_8_mut = tail(x_8)
                    continue
    ...
    def cg_expr(action_name: str, scope: FSharpList[Tuple[str, str]], expr: expr_1, analyzer=analyzer, _arg1=_arg1, stmts=stmts) -> DocBuilder_block_1[Doc]:
        def arrow_107(action_name=action_name, scope=scope, expr=expr) -> DocBuilder_block_1[Doc]:
            match_value_1 : node = expr.node
            if match_value_1.tag == 6:
                match_value_2 : Optional[str] = try_lookup(match_value_1.fields[0], scope)
                def arrow_103(_unit=None) -> Doc:
                    raise UnboundVariable(match_value_1.fields[0])
                
                return DocBuilder_Builder__Return_1505(DocBuilder_cg, word(match_value_2)) if (match_value_2 is not None) else (DocBuilder_Builder__Return_1505(DocBuilder_cg, arrow_103()))
                ...
                def arrow_105(_unit=None) -> str:
                    arg20_1 : int = (match_value_1.fields[0] - 1) or 0
                    return to_text(printf("%s[%d]"))(TREE_NAME)(arg20_1)
                
                def arrow_106(_arg11: FSharpList[Doc]) -> DocBuilder_block_1[Doc]:
    ...
    def mk_lexer_debug(def__1: lexerule, analyzer=analyzer, _arg1=_arg1, stmts=stmts) -> str:
    ...
    def cg_stmt(stmt: definition_1, analyzer=analyzer, _arg1=_arg1, stmts=stmts) -> Doc:
        ...
        if stmt.tag == 2:
            ...
            idx_1 : int = 0
            def arrow_110(stmt=stmt) -> Any:
                def arrow_109(match_value_3: Tuple[position, production_1]) -> Any:
                    ...
                    return append_1(singleton(Doc_op_Addition_Z7CFFAC00(Doc_op_Addition_Z7CFFAC00(seplist(word(" "), map_1(lambda arg_2: word(cg_symbol(arg_2)), production.symbols)), word("-\u003e")), word(action_name_2))), delay(arrow_108))
                
                return collect(arrow_109, decl["define"])
            
            body_4 : Doc = align(vsep(map_indexed(lambda i_6, e_1, stmt=stmt: Doc_op_Addition_Z7CFFAC00(word(":") if (i_6 == 0) else (word("|")), e_1), to_list(delay(arrow_110)))))
    ...
    class ObjectExpr111:
        @property
        def Compare(self) -> Any:
            return lambda x_15, y_8: compare_primitives(x_15, y_8)
        
    sort_in_place_by(lambda k_1, analyzer=analyzer, _arg1=_arg1, stmts=stmts: find_index(lambda y_7: k_1 == y_7, analyzer.TokenFragments), ReferencedNamedTokens, ObjectExpr111())
    ...
    class ObjectExpr112:
        @property
        def Compare(self) -> Any:
            return lambda x_16, y_9: compare_primitives(x_16, y_9)
        
    arr : List[str] = sort(list(analyzer.LiteralTokens), ObjectExpr112())
    ...
    def arrow_122(analyzer=analyzer, _arg1=_arg1, stmts=stmts) -> Any:
        def arrow_121(_unit=None) -> Any:
            def arrow_120(_unit=None) -> Any:
                names : Doc = parens(seplist(word(","), map_1(lambda s_13: word(s_13), import_names)))
                def arrow_119(_unit=None) -> Any:
                    def arrow_118(_unit=None) -> Any:
