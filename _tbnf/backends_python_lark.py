...
from .FableSedlex.sedlex import (Automata_node, pcompl, pinterval, pplus, pstar, pany, pseq, por, popt, pstring, keep_token, build)
...
def codegen(analyzer: Analyzer, _arg1: CodeGenOptions, stmts: List[definition_1]) -> List[Tuple[str, Doc]]:
    ...
    export_names : Any = of_array([export_parser, export_grammar], ObjectExpr97())
    class ObjectExpr98:
        @property
        def Compare(self) -> Any:
            return lambda x_1, y_1: compare_primitives(x_1, y_1)
        
    abandoned_names : Any = of_array(["False", "None", "True", "and", "as", "assert", "async", "await", "break", "class", "continue", "def", "del", "elif", "else", "except", "finally", "for", "from", "global", "if", "import", "in", "is", "lambda", "nonlocal", "not", "or", "pass", "raise", "return", "try", "while", "with", "yield", "match", "case"], ObjectExpr98())
    class ObjectExpr99:
        @property
        def Compare(self) -> Any:
            return lambda x_2, y_2: compare(x_2, y_2)
        
    symmap : Any = empty_1(ObjectExpr99())
    ...
    lexer_maps : Any = empty_1(ObjectExpr100())
    ...
    name_env : NameMangling_nameEnv = NameMangling_nameEnv(of_list(map_1(lambda tuple, analyzer=analyzer, _arg1=_arg1, stmts=stmts: tuple[1], global_scope), ObjectExpr101()))
    mangle = None
    ...
    def cg_expr(action_name: str, scope: FSharpList[Tuple[str, str]], expr: expr_1, analyzer=analyzer, _arg1=_arg1, stmts=stmts) -> DocBuilder_block_1[Doc]:
        ...
        def arrow_106(action_name=action_name, scope=scope, expr=expr) -> DocBuilder_block_1[Doc]:
            match_value_1 : node = expr.node
            if match_value_1.tag == 6:
                match_value_2 : Optional[str] = try_lookup(match_value_1.fields[0], scope)
                def arrow_102(_unit=None) -> Doc:
                    raise UnboundVariable(match_value_1.fields[0])
                
                return DocBuilder_Builder__Return_1505(DocBuilder_cg, word(match_value_2)) if (match_value_2 is not None) else (DocBuilder_Builder__Return_1505(DocBuilder_cg, arrow_102()))
                ...
                def arrow_103(_arg7: Doc) -> DocBuilder_block_1[Doc]:
                ...
                def arrow_104(_unit=None) -> str:
                    arg20_1 : int = (match_value_1.fields[0] - 1) or 0
                    return to_text(printf("%s[%d]"))(TREE_NAME)(arg20_1)
                
                def arrow_105(_arg11: FSharpList[Doc]) -> DocBuilder_block_1[Doc]:
    ...
    def mk_lexer(def__1: lexerule, analyzer=analyzer, _arg1=_arg1, stmts=stmts) -> Callable[[Automata_node], Automata_node]:
        ...
        if def__1.tag == 9:
            ...
            return pstring(def__1.fields[0])
    ...
    def cg_stmt(stmt: definition_1, analyzer=analyzer, _arg1=_arg1, stmts=stmts) -> Doc:
        ...
        if stmt.tag == 2:
            ...
            idx_1 : int = 0
            def arrow_109(stmt=stmt) -> Any:
                def arrow_108(match_value_3: Tuple[position, production_1]) -> Any:
                    ...
                    return append_1(singleton(Doc_op_Addition_Z7CFFAC00(Doc_op_Addition_Z7CFFAC00(seplist(word(" "), map_1(lambda arg_2: word(cg_symbol(arg_2)), production.symbols)), word("-\u003e")), word(action_name_2))), delay(arrow_107))
                
                return collect(arrow_108, decl["define"])
            
            body_4 : Doc = align(vsep(map_indexed(lambda i_2, e_1, stmt=stmt: Doc_op_Addition_Z7CFFAC00(word(":") if (i_2 == 0) else (word("|")), e_1), to_list(delay(arrow_109)))))
    ...
    sort_in_place_by(lambda k_1, analyzer=analyzer, _arg1=_arg1, stmts=stmts: find_index(lambda y_5: k_1 == y_5, analyzer.TokenFragments), ReferencedNamedTokens, ObjectExpr110())
    ...
    class ObjectExpr111:
        @property
        def Compare(self) -> Any:
            return lambda x_8, y_7: compare_primitives(x_8, y_7)
        
    arr : List[str] = sort(list(analyzer.LiteralTokens), ObjectExpr111())
    for idx_4 in range(0, (len(arr) - 1) + 1, 1):
        ...
        token_names = cons(cg_symbol(symbol(0, {
            "define": k_3,
            "is_literal": True
        })), token_names)
        token_id = (token_id + 1) or 0
        idx_2 = (idx_2 + 1) or 0
    lexer_info_1 : List[Tuple[Callable[[Automata_node], Automata_node], keep_token]] = to_array(lexer_info)
    token_names_1 : FSharpList[Doc] = map_1(lambda arg_3, analyzer=analyzer, _arg1=_arg1, stmts=stmts: word(escape_string(arg_3)), token_names)
    def arrow_120(analyzer=analyzer, _arg1=_arg1, stmts=stmts) -> Any:
