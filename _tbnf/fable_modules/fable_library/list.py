...
from typing import (TypeVar, Optional, Tuple, Callable, Any, Iterator, Generic, List, Iterable)
...
from .util import (equals, structural_hash, compare, to_iterator, get_enumerator, IDisposable, ignore, is_array_like, uncurry, IEqualityComparer)
...
def FSharpList__get_Length(xs: FSharpList[Any]) -> int:
    def loop(i_mut: int, xs_1_mut: FSharpList[Any], xs: FSharpList[T]=xs) -> int:
        while True:
...
def FSharpList__get_Item_Z524259A4(xs: FSharpList[T], index: int) -> T:
    def loop(i_mut: int, xs_1_mut: FSharpList[_50308_], xs: FSharpList[T]=xs, index: int=index) -> _50308_:
        while True:
            (i, xs_1) = (i_mut, xs_1_mut)
...
def compare_with(comparer: Callable[[T, T], int], xs: FSharpList[T], ys: FSharpList[T]) -> int:
    def loop(xs_1_mut: FSharpList[T], ys_1_mut: FSharpList[T], comparer: Callable[[T, T], int]=comparer, xs: FSharpList[T]=xs, ys: FSharpList[T]=ys) -> int:
        while True:
            (xs_1, ys_1) = (xs_1_mut, ys_1_mut)
...
def to_array(xs: FSharpList[T]) -> List[T]:
    ...
    def loop(i_mut: int, xs_1_mut: FSharpList[Any], xs: FSharpList[T]=xs) -> None:
        while True:
            (i, xs_1) = (i_mut, xs_1_mut)
...
def fold_indexed(folder: Callable[[int, State, T], State], state: State, xs: FSharpList[T]) -> State:
    def loop(i_mut: int, acc_mut: State, xs_1_mut: FSharpList[Any], folder: Callable[[int, State, T], State]=folder, state: State=state, xs: FSharpList[T]=xs) -> State:
        while True:
...
def unfold(gen: Callable[[State], Optional[Tuple[T, State]]], state: State=None) -> FSharpList[T]:
    def loop(acc_mut: Any, node_mut: FSharpList[T], gen: Callable[[State], Optional[Tuple[T, State]]]=gen, state: State=state) -> FSharpList[T]:
        while True:
            (acc, node) = (acc_mut, node_mut)
            match_value : Optional[Tuple[T, State]] = gen(acc)
            if match_value is not None:
                acc_mut = match_value[1]
                def arrow_93(acc: State=acc, node: FSharpList[T]=node) -> FSharpList[Any]:
...
def iterate(action: Callable[[a_], None], xs: FSharpList[a_]) -> None:
    def arrow_94(unit_var0: None, x: Any=None, action: Callable[[a_], None]=action, xs: FSharpList[a_]=xs) -> None:
        action(x)
    
    fold(arrow_94, None, xs)


def iterate2(action: Callable[[a_, b_], None], xs: FSharpList[a_], ys: FSharpList[b_]) -> None:
    def arrow_95(unit_var0: None, x: Any, y: Any=None, action: Callable[[a_, b_], None]=action, xs: FSharpList[a_]=xs, ys: FSharpList[b_]=ys) -> None:
        action(x, y)
    
    fold2(arrow_95, None, xs, ys)


def iterate_indexed(action: Callable[[int, a_], None], xs: FSharpList[a_]) -> None:
    def arrow_96(i: int, x: Any=None, action: Callable[[int, a_], None]=action, xs: FSharpList[a_]=xs) -> int:
        action(i, x)
        return i + 1
    
    ignore(fold(arrow_96, 0, xs))


def iterate_indexed2(action: Callable[[int, a_, b_], None], xs: FSharpList[a_], ys: FSharpList[b_]) -> None:
    def arrow_97(i: int, x: Any, y: Any=None, action: Callable[[int, a_, b_], None]=action, xs: FSharpList[a_]=xs, ys: FSharpList[b_]=ys) -> int:
        action(i, x, y)
        return i + 1
    
    ignore(fold2(arrow_97, 0, xs, ys))


def to_seq(xs: FSharpList[T]) -> Iterable[T]:
    return xs
...
def of_seq(xs: Iterable[T]) -> FSharpList[T]:
    if is_array_like(xs):
        return of_array(xs)
        ...
        with get_enumerator(xs) as enumerator:
            while enumerator.System_Collections_IEnumerator_MoveNext():
                ...
                def arrow_99(xs: Iterable[T]=xs) -> FSharpList[Any]:
...
def concat(lists: Iterable[FSharpList[T]]) -> FSharpList[T]:
    root : FSharpList[T] = FSharpList_get_Empty()
    node : FSharpList[T] = root
    def action(xs: FSharpList[Any], lists: Iterable[FSharpList[T]]=lists) -> None:
        nonlocal node
        def arrow_101(acc: FSharpList[T], x: T=None, xs: FSharpList[T]=xs) -> FSharpList[T]:
...
def scan(folder: Callable[[State, T], State], state: State, xs: FSharpList[T]) -> FSharpList[State]:
    ...
    def arrow_102(folder: Callable[[State, T], State]=folder, state: State=state, xs: FSharpList[T]=xs) -> FSharpList[Any]:
    ...
    while not FSharpList__get_IsEmpty(xs_3):
        ...
        def arrow_104(folder: Callable[[State, T], State]=folder, state: State=state, xs: FSharpList[T]=xs) -> FSharpList[Any]:
...
def collect(mapping: Callable[[T], FSharpList[U]], xs: FSharpList[T]) -> FSharpList[U]:
    ...
    while not FSharpList__get_IsEmpty(ys):
        ...
        while not FSharpList__get_IsEmpty(zs):
            def arrow_106(mapping: Callable[[T], FSharpList[U]]=mapping, xs: FSharpList[T]=xs) -> FSharpList[Any]:
...
def map_indexed(mapping: Callable[[int, T], U], xs: FSharpList[T]) -> FSharpList[U]:
    root : FSharpList[U] = FSharpList_get_Empty()
    def folder(i: int, acc: FSharpList[U], x: Any=None, mapping: Callable[[int, T], U]=mapping, xs: FSharpList[T]=xs) -> FSharpList[U]:
        def arrow_107(i: int=i, acc: FSharpList[U]=acc, x: T=x) -> FSharpList[Any]:
...
def map(mapping: Callable[[T], U], xs: FSharpList[T]) -> FSharpList[U]:
    root : FSharpList[U] = FSharpList_get_Empty()
    def folder(acc: FSharpList[U], x: Any=None, mapping: Callable[[T], U]=mapping, xs: FSharpList[T]=xs) -> FSharpList[U]:
        def arrow_108(acc: FSharpList[U]=acc, x: T=x) -> FSharpList[Any]:
...
def map2(mapping: Callable[[T1, T2], U], xs: FSharpList[T1], ys: FSharpList[T2]) -> FSharpList[U]:
    root : FSharpList[U] = FSharpList_get_Empty()
    def folder(acc: FSharpList[U], x: Any, y: Any=None, mapping: Callable[[T1, T2], U]=mapping, xs: FSharpList[T1]=xs, ys: FSharpList[T2]=ys) -> FSharpList[U]:
        def arrow_109(acc: FSharpList[U]=acc, x: T1=x, y: T2=y) -> FSharpList[Any]:
...
def map_indexed2(mapping: Callable[[int, T1, T2], U], xs: FSharpList[T1], ys: FSharpList[T2]) -> FSharpList[U]:
    def loop(i_mut: int, acc_mut: FSharpList[U], xs_1_mut: FSharpList[Any], ys_1_mut: FSharpList[Any], mapping: Callable[[int, T1, T2], U]=mapping, xs: FSharpList[T1]=xs, ys: FSharpList[T2]=ys) -> FSharpList[U]:
        while True:
            ...
            if True if (FSharpList__get_IsEmpty(xs_1)) else (FSharpList__get_IsEmpty(ys_1)):
                ...
                i_mut = i + 1
                def arrow_110(i: int=i, acc: FSharpList[U]=acc, xs_1: FSharpList[T1]=xs_1, ys_1: FSharpList[T2]=ys_1) -> FSharpList[Any]:
...
def map3(mapping: Callable[[T1, T2, T3], U], xs: FSharpList[T1], ys: FSharpList[T2], zs: FSharpList[T3]) -> FSharpList[U]:
    def loop(acc_mut: FSharpList[U], xs_1_mut: FSharpList[Any], ys_1_mut: FSharpList[Any], zs_1_mut: FSharpList[Any], mapping: Callable[[T1, T2, T3], U]=mapping, xs: FSharpList[T1]=xs, ys: FSharpList[T2]=ys, zs: FSharpList[T3]=zs) -> FSharpList[U]:
        while True:
            ...
            if True if (True if (FSharpList__get_IsEmpty(xs_1)) else (FSharpList__get_IsEmpty(ys_1))) else (FSharpList__get_IsEmpty(zs_1)):
                ...
                def arrow_111(acc: FSharpList[U]=acc, xs_1: FSharpList[T1]=xs_1, ys_1: FSharpList[T2]=ys_1, zs_1: FSharpList[T3]=zs_1) -> FSharpList[Any]:
...
def map_fold(mapping: Callable[[State, T], Tuple[Result, State]], state: State, xs: FSharpList[T]) -> Tuple[FSharpList[Result], State]:
    ...
    def folder(tupled_arg: Tuple[FSharpList[Result], State], x: Any=None, mapping: Callable[[State, T], Tuple[Result, State]]=mapping, state: State=state, xs: FSharpList[T]=xs) -> Tuple[FSharpList[Result], State]:
        pattern_input : Tuple[Result, State] = mapping(tupled_arg[1], x)
        def arrow_112(tupled_arg: Tuple[FSharpList[Result], State]=tupled_arg, x: T=x) -> FSharpList[Any]:
...
def try_pick(f: Callable[[T], Optional[a_]], xs: FSharpList[T]) -> Optional[a_]:
    def loop(xs_1_mut: FSharpList[Any], f: Callable[[T], Optional[a_]]=f, xs: FSharpList[T]=xs) -> Optional[a_]:
        while True:
            (xs_1,) = (xs_1_mut,)
            if FSharpList__get_IsEmpty(xs_1):
                return None
...
def pick(f: Callable[[a_], Optional[b_]], xs: FSharpList[a_]) -> b_:
    ...
    if match_value is None:
        def arrow_113(f: Callable[[a_], Optional[b_]]=f, xs: FSharpList[a_]=xs) -> Any:
...
def find(f: Callable[[a_], bool], xs: FSharpList[a_]) -> a_:
    ...
    if match_value is None:
        def arrow_114(f: Callable[[a_], bool]=f, xs: FSharpList[a_]=xs) -> Any:
            raise Exception(SR_keyNotFoundAlt)
...
def find_back(f: Callable[[a_], bool], xs: FSharpList[a_]) -> a_:
    ...
    if match_value is None:
        def arrow_115(f: Callable[[a_], bool]=f, xs: FSharpList[a_]=xs) -> Any:
            raise Exception(SR_keyNotFoundAlt)
...
def try_find_index(f: Callable[[T], bool], xs: FSharpList[T]) -> Optional[int]:
    def loop(i_mut: int, xs_1_mut: FSharpList[Any], f: Callable[[T], bool]=f, xs: FSharpList[T]=xs) -> Optional[int]:
        while True:
            (i, xs_1) = (i_mut, xs_1_mut)
            if FSharpList__get_IsEmpty(xs_1):
                return None
...
def find_index(f: Callable[[a_], bool], xs: FSharpList[a_]) -> int:
    ...
    if match_value is None:
        def arrow_116(f: Callable[[a_], bool]=f, xs: FSharpList[a_]=xs) -> int:
            raise Exception(SR_keyNotFoundAlt)
...
def find_index_back(f: Callable[[a_], bool], xs: FSharpList[a_]) -> int:
    ...
    if match_value is None:
        def arrow_117(f: Callable[[a_], bool]=f, xs: FSharpList[a_]=xs) -> int:
            raise Exception(SR_keyNotFoundAlt)
...
def try_item(n: int, xs: FSharpList[T]) -> Optional[T]:
    def loop(i_mut: int, xs_1_mut: FSharpList[T], n: int=n, xs: FSharpList[T]=xs) -> Optional[T]:
        while True:
            (i, xs_1) = (i_mut, xs_1_mut)
            if FSharpList__get_IsEmpty(xs_1):
                return None
...
def filter(f: Callable[[T], bool], xs: FSharpList[T]) -> FSharpList[T]:
    root : FSharpList[T] = FSharpList_get_Empty()
    def folder(acc: FSharpList[T], x: T=None, f: Callable[[T], bool]=f, xs: FSharpList[T]=xs) -> FSharpList[T]:
        if f(x):
            def arrow_118(acc: FSharpList[T]=acc, x: T=x) -> FSharpList[Any]:
                t : FSharpList[T] = FSharpList(x, None)
                acc.tail = t
                return t
...
def partition(f: Callable[[T], bool], xs: FSharpList[T]) -> Tuple[FSharpList[T], FSharpList[T]]:
    ...
    def folder(tupled_arg: Tuple[FSharpList[T], FSharpList[T]], f: Callable[[T], bool]=f, xs: FSharpList[T]=xs) -> Callable[[T], Tuple[FSharpList[T], FSharpList[T]]]:
        lacc : FSharpList[T] = tupled_arg[0]
        racc : FSharpList[T] = tupled_arg[1]
        def arrow_121(x: T=None, tupled_arg: Tuple[FSharpList[T], FSharpList[T]]=tupled_arg) -> Tuple[FSharpList[T], FSharpList[T]]:
...
def choose(f: Callable[[T], Optional[T]], xs: FSharpList[T]) -> FSharpList[T]:
    root : FSharpList[T] = FSharpList_get_Empty()
    def folder(acc: FSharpList[T], x: T=None, f: Callable[[T], Optional[T]]=f, xs: FSharpList[T]=xs) -> FSharpList[T]:
        match_value : Optional[T] = f(x)
        if match_value is None:
            return acc
        
        else: 
            def arrow_122(acc: FSharpList[T]=acc, x: T=x) -> FSharpList[Any]:
...
def contains(value: T, xs: FSharpList[T], eq: IEqualityComparer[Any]) -> bool:
...
def initialize(n: int, f: Callable[[int], T]) -> FSharpList[T]:
    ...
    for i in range(0, (n - 1) + 1, 1):
        def arrow_124(n: int=n, f: Callable[[int], T]=f) -> FSharpList[Any]:
...
def sort(xs: FSharpList[T], comparer: IComparer_1[T]) -> FSharpList[T]:
    return sort_with(lambda x, y=None, xs=xs, comparer=comparer: comparer.Compare(x, y), xs)


def sort_by(projection: Callable[[T], U], xs: FSharpList[T], comparer: IComparer_1[U]) -> FSharpList[T]:
...
def sort_descending(xs: FSharpList[T], comparer: IComparer_1[T]) -> FSharpList[T]:
    return sort_with(lambda x, y=None, xs=xs, comparer=comparer: comparer.Compare(x, y) * -1, xs)


def sort_by_descending(projection: Callable[[T], U], xs: FSharpList[T], comparer: IComparer_1[U]) -> FSharpList[T]:
...
def sum(xs: FSharpList[T], adder: IGenericAdder_1[T]) -> T:
    return fold(lambda acc, x=None, xs=xs, adder=adder: adder.Add(acc, x), adder.GetZero(), xs)


def sum_by(f: Callable[[T], U], xs: FSharpList[T], adder: IGenericAdder_1[U]) -> U:
    return fold(lambda acc, x=None, f=f, xs=xs, adder=adder: adder.Add(acc, f(x)), adder.GetZero(), xs)


def max_by(projection: Callable[[T], U], xs: FSharpList[T], comparer: IComparer_1[U]) -> T:
...
def max(xs: FSharpList[T], comparer: IComparer_1[T]) -> T:
    return reduce(lambda x, y=None, xs=xs, comparer=comparer: y if (comparer.Compare(y, x) > 0) else (x), xs)


def min_by(projection: Callable[[T], U], xs: FSharpList[T], comparer: IComparer_1[U]) -> T:
...
def min(xs: FSharpList[T], comparer: IComparer_1[T]) -> T:
    return reduce(lambda x, y=None, xs=xs, comparer=comparer: x if (comparer.Compare(y, x) > 0) else (y), xs)


def average(xs: FSharpList[T], averager: IGenericAverager_1[T]) -> T:
    count : int = 0
    def folder(acc: T, x: T=None, xs: FSharpList[T]=xs, averager: IGenericAverager_1[T]=averager) -> T:
        nonlocal count
        count = (count + 1) or 0
...
def average_by(f: Callable[[T], U], xs: FSharpList[T], averager: IGenericAverager_1[U]) -> U:
    count : int = 0
    def arrow_125(acc: U, x: Any=None, f: Callable[[T], U]=f, xs: FSharpList[T]=xs, averager: IGenericAverager_1[U]=averager) -> U:
        nonlocal count
        count = (count + 1) or 0
        return averager.Add(acc, f(x))
    
    return averager.DivideByInt(fold(arrow_125, averager.GetZero(), xs), count)
...
def all_pairs(xs: FSharpList[T1], ys: FSharpList[T2]) -> FSharpList[Tuple[T1, T2]]:
    ...
    def arrow_129(x: Any=None, xs: FSharpList[T1]=xs, ys: FSharpList[T2]=ys) -> None:
        def arrow_128(y: Any=None) -> None:
            nonlocal node
            def arrow_127(_unit=None) -> FSharpList[Tuple[T1, T2]]:
                xs_1 : FSharpList[Tuple[T1, T2]] = node
                def arrow_126(_unit=None) -> FSharpList[Any]:
        ...
        iterate(arrow_128, ys)
    
    iterate(arrow_129, xs)
...
def take(count: int, xs: FSharpList[T]) -> FSharpList[T]:
    ...
    def loop(i_mut: int, acc_mut: FSharpList[T], xs_1_mut: FSharpList[T], count: int=count, xs: FSharpList[T]=xs) -> FSharpList[T]:
        while True:
            (i, acc, xs_1) = (i_mut, acc_mut, xs_1_mut)
            if i <= 0:
                return acc
                ...
                i_mut = i - 1
                def arrow_130(i: int=i, acc: FSharpList[T]=acc, xs_1: FSharpList[T]=xs_1) -> FSharpList[Any]:
...
def take_while(predicate: Callable[[T], bool], xs: FSharpList[T]) -> FSharpList[T]:
    def loop(acc_mut: FSharpList[T], xs_1_mut: FSharpList[T], predicate: Callable[[T], bool]=predicate, xs: FSharpList[T]=xs) -> FSharpList[T]:
        while True:
            ...
            if FSharpList__get_IsEmpty(xs_1):
                ...
                def arrow_131(acc: FSharpList[T]=acc, xs_1: FSharpList[T]=xs_1) -> FSharpList[Any]:
                    t : FSharpList[T] = FSharpList(FSharpList__get_Head(xs_1), None)
                    acc.tail = t
                    return t
    ...
    def loop(i_mut: int, acc_mut: FSharpList[T], xs_1_mut: FSharpList[T], count: int=count, xs: FSharpList[T]=xs) -> FSharpList[T]:
        while True:
            ...
            if i <= 0:
                ...
                i_mut = i - 1
                def arrow_132(i: int=i, acc: FSharpList[T]=acc, xs_1: FSharpList[T]=xs_1) -> FSharpList[Any]:
                    t : FSharpList[T] = FSharpList(FSharpList__get_Head(xs_1), None)
                    acc.tail = t
                    return t
...
def transpose(lists: Iterable[FSharpList[T]]) -> FSharpList[FSharpList[T]]:
...
def insert_at(index: int, y: T, xs: FSharpList[T]) -> FSharpList[T]:
    i : int = -1
    is_done : bool = False
    def folder(acc: FSharpList[T], x: T=None, index: int=index, y: T=y, xs: FSharpList[T]=xs) -> FSharpList[T]:
        nonlocal i, is_done
        i = (i + 1) or 0
        if i == index:
            is_done = True
    ...
    def arrow_133(index: int=index, y: T=y, xs: FSharpList[T]=xs) -> FSharpList[Any]:
        raise Exception((SR_indexOutOfBounds + "\\nParameter name: ") + "index")
    
    return reverse(result if (is_done) else (FSharpList_Cons_305B8EAC(y, result) if ((i + 1) == index) else (arrow_133())))


def insert_many_at(index: int, ys: Iterable[T], xs: FSharpList[T]) -> FSharpList[T]:
    i : int = -1
    is_done : bool = False
    ys_1 : FSharpList[T] = of_seq(ys)
    def folder(acc: FSharpList[T], x: T=None, index: int=index, ys: Iterable[T]=ys, xs: FSharpList[T]=xs) -> FSharpList[T]:
        nonlocal i, is_done
        i = (i + 1) or 0
        if i == index:
            is_done = True
    ...
    result : FSharpList[T] = fold(folder, FSharpList_get_Empty(), xs)
    def arrow_134(index: int=index, ys: Iterable[T]=ys, xs: FSharpList[T]=xs) -> FSharpList[Any]:
        raise Exception((SR_indexOutOfBounds + "\\nParameter name: ") + "index")
    
    return reverse(result if (is_done) else (append(ys_1, result) if ((i + 1) == index) else (arrow_134())))


def remove_at(index: int, xs: FSharpList[T]) -> FSharpList[T]:
    i : int = -1
    is_done : bool = False
    def f(_arg1: Any=None, index: int=index, xs: FSharpList[T]=xs) -> bool:
        nonlocal i, is_done
        i = (i + 1) or 0
        if i == index:
            is_done = True
...
def remove_many_at(index: int, count: int, xs: FSharpList[T]) -> FSharpList[T]:
    i : int = -1
    status : int = -1
    def f(_arg1: Any=None, index: int=index, count: int=count, xs: FSharpList[T]=xs) -> bool:
        nonlocal i, status
        i = (i + 1) or 0
        if i == index:
            status = 0
...
def update_at(index: int, y: T, xs: FSharpList[T]) -> FSharpList[T]:
    is_done : bool = False
    def mapping(i: int, x: T=None, index: int=index, y: T=y, xs: FSharpList[T]=xs) -> T:
        nonlocal is_done
        if i == index:
            is_done = True
            return y
