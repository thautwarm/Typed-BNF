...
def codegen(analyzer: Analyzer, cg_options: CodeGenOptions, lang_name: str, stmts: List[definition]) -> List[Tuple[str, Doc]]:
    ...
    abandoned_names : Any = of_array(["and", "as", "assert", "asr", "begin", "class", "constraint", "do", "done", "downto", "else", "end", "exception", "external", "false", "for", "fun", "function", "functor", "if", "in", "include", "inherit", "initializer", "land", "lazy", "let", "lor", "lsl", "lsr", "lxor", "match", "method", "mod", "module", "mutable", "new", "nonrec", "object", "of", "open", "or", "private", "rec", "sig", "struct", "then", "to", "true", "try", "type", "val", "virtual", "when", "while", "with"], ObjectExpr117())
    class ObjectExpr118:
        @property
        def Compare(self) -> Any:
            return lambda x_5, y_1: compare(x_5, y_1)
        
    symmap : Any = empty_1(ObjectExpr118())
    ...
    def arrow_120(i: int, c: str, analyzer: Analyzer=analyzer, cg_options: CodeGenOptions=cg_options, lang_name: str=lang_name, stmts: List[definition]=stmts) -> bool:
        test : bool = True if (True if (is_lower(c)) else (is_unicode(c))) else (c == "_")
        return test if (i == 0) else (True if (test) else (is_digit(c)))
    
    ocaml_var_ident_descr : NameMangling_IdentifierDescriptor = NameMangling_IdentifierDescriptor__WithNameEnv_Z7613F24B(NameMangling_IdentifierDescriptor_Create_Z48C5CCEF(arrow_120, lambda i_1, c_1, analyzer=analyzer, cg_options=cg_options, lang_name=lang_name, stmts=stmts: to_text(interpolate("_X%P()_", [i_1])) if (is_digit(c_1)) else (c_1.lower() if (is_upper(c_1)) else (to_text(interpolate("_%P()_", [ord(c_1)]))))), NameMangling_nameEnv(of_list(map_1(lambda tuple, analyzer=analyzer, cg_options=cg_options, lang_name=lang_name, stmts=stmts: tuple[1], global_scope), ObjectExpr119())))
    class ObjectExpr121:
        @property
        def Compare(self) -> Any:
            return lambda x_7, y_3: compare_primitives(x_7, y_3)
        
    sedlex_ident_descr : NameMangling_IdentifierDescriptor = NameMangling_IdentifierDescriptor__WithNameEnv_Z7613F24B(NameMangling_IdentifierDescriptor_Create_Z48C5CCEF(lambda i_2, c_2, analyzer=analyzer, cg_options=cg_options, lang_name=lang_name, stmts=stmts: is_upper(c_2) if (i_2 == 0) else (True if (True if (is_upper(c_2)) else (c_2 == "_")) else (is_digit(c_2))), lambda i_3, c_3, analyzer=analyzer, cg_options=cg_options, lang_name=lang_name, stmts=stmts: c_3.upper() if (is_lower(c_3)) else (NameMangling_maskChar(ord("A"), ord("Z"), ord(c_3)) + "_" if (i_3 == 0) else (("_" + NameMangling_maskChar(ord("A"), ord("Z"), ord(c_3))) + "_"))), NameMangling_nameEnv(of_array(["EOF"], ObjectExpr121())))
    class ObjectExpr122:
        @property
        def Compare(self) -> Any:
    ...
    def cg_expr(scope: FSharpList[Tuple[str, str]], curr_expr: expr, analyzer: Analyzer=analyzer, cg_options: CodeGenOptions=cg_options, lang_name: str=lang_name, stmts: List[definition]=stmts) -> Doc:
        ...
        if match_value_1.tag == 6:
            ...
            def arrow_129(scope: FSharpList[Tuple[str, str]]=scope, curr_expr: expr=curr_expr) -> Iterable[Tuple[str, str]]:
                def arrow_128(match_value_3: Tuple[str, monot]) -> Iterable[Tuple[str, str]]:
                    arg : str = match_value_3[0]
                    return singleton((arg, mangle(ocaml_var_ident_descr, arg)))
                
                return collect(arrow_128, match_value_1.fields[0])
            
            code : Doc = cg_expr(append(to_list(delay(arrow_129)), scope), match_value_1.fields[1])
    ...
    def cg_stmt(stmt: definition, analyzer: Analyzer=analyzer, cg_options: CodeGenOptions=cg_options, lang_name: str=lang_name, stmts: List[definition]=stmts) -> Doc:
        ...
        if stmt.tag == 2:
            ...
            def arrow_133(stmt: definition=stmt) -> Iterable[Doc]:
                def arrow_132(match_value_4: Tuple[position, production_1]) -> Iterable[Doc]:
                    ...
                    return singleton(arrow_131())
                
                return collect(arrow_132, decl["define"])
            
            body_4 : Doc = align(vsep(map_indexed(lambda i_7, e_1, stmt=stmt: Doc_op_Addition_Z7CFFAC00(word(":") if (i_7 == 0) else (word("|")), e_1), to_list(delay(arrow_133)))))
    ...
    def arrow_166(analyzer: Analyzer=analyzer, cg_options: CodeGenOptions=cg_options, lang_name: str=lang_name, stmts: List[definition]=stmts) -> Iterable[Doc]:
        doc_ctor_wrap_funcs : FSharpList[Tuple[str, str, FSharpList[Tuple[str, str]], Doc]] = empty()
        def arrow_164(_unit=None) -> Iterable[Doc]:
            def arrow_163(_unit=None) -> Iterable[Doc]:
                adt_cases : FSharpList[Tuple[str, Any]] = Sigma__GetADTCases(analyzer.Sigma)
                def arrow_162(_unit=None) -> Iterable[Doc]:
                    def arrow_161(_unit=None) -> Iterable[Doc]:
                        def arrow_140(match_value_6: Tuple[str, Any]) -> Iterable[Doc]:
                            typename_0027 : str = type_renamer(match_value_6[0])
                            def arrow_139(_unit=None) -> Iterable[Doc]:
                                def arrow_138(match_value_7: Tuple[str, FSharpList[Tuple[str, monot]]]) -> Iterable[Doc]:
                                ...
                                return append_1(collect(arrow_138, to_array_1(match_value_6[1])), delay(lambda _unit=None: singleton(empty_2)))
                            
                            return append_1(singleton(word(to_text(interpolate("and %P() = ", [typename_0027])))), delay(arrow_139))
                        ...
                        def arrow_160(_unit=None) -> Iterable[Doc]:
                            def arrow_154(match_value_9: Tuple[str, Shape]) -> Iterable[Doc]:
                                ...
                                fields_2 : FSharpList[Tuple[str, str]] = to_list(delay(lambda _unit=None: collect(lambda match_value_10: singleton((field_renamer(match_value_10[0]), cg_type(match_value_10[1]))), to_array_1(shape.fields))))
                                ...
                                def arrow_153(_unit=None) -> Iterable[Doc]:
                                    def arrow_152(_unit=None) -> Iterable[Doc]:
                                        ...
                                        return append_1(singleton(word("}")), delay(arrow_151))
                                    
                                    return append_1(singleton(Doc_op_RightShift_2AAA0F3C(vsep(to_list(delay(lambda _unit=None: collect(lambda match_value_11: singleton(Doc_op_Addition_Z7CFFAC00(Doc_op_Addition_Z7CFFAC00(word(match_value_11[0]), word(":")), Doc_op_Multiply_Z7CFFAC00(word(match_value_11[1]), word(";")))), fields_2)))), 4)), delay(arrow_152))
                                
                                return append_1(singleton(Doc_op_Addition_Z7CFFAC00(Doc_op_Addition_Z7CFFAC00(Doc_op_Addition_Z7CFFAC00(Doc_op_Addition_Z7CFFAC00(word("and"), ret_t_1), word("=")), word("MK_" + typename_0027_1)), word("of unit"))), delay(arrow_142)) if (is_empty(fields_2)) else (append_1(singleton(Doc_op_Addition_Z7CFFAC00(Doc_op_Addition_Z7CFFAC00(Doc_op_Addition_Z7CFFAC00(word("and"), ret_t_1), word("=")), word("{"))), delay(arrow_153)))
                            
                            def arrow_159(_unit=None) -> Iterable[Doc]:
                                def arrow_158(match_value_12: Tuple[str, str, FSharpList[Tuple[str, str]], Doc]) -> Iterable[Doc]:
                                ...
                                return collect(arrow_158, doc_ctor_wrap_funcs)
                            
                            return append_1(collect(arrow_154, Sigma__GetRecordTypes(analyzer.Sigma)), delay(arrow_159))
                        
                        return append_1(collect(arrow_140, adt_cases), delay(arrow_160))
                    
                    return append_1(singleton(word("type ___used_t_head_90xasda")), delay(arrow_161))
                
                return append_1(singleton(empty_2), delay(arrow_162))
            
            return append_1(singleton(word(to_text(interpolate("open %P();;", [capitalized(filename_lexer)])))), delay(arrow_163))
        
        return append_1(singleton(word(to_text(interpolate("open %P();;", [capitalized(filename_require)])))), delay(arrow_164))
    
    file_constructors : Tuple[str, Doc] = (filename_constructors + ".ml", vsep(to_list(delay(arrow_166))))
    ...
    sort_in_place_by(lambda k_1, analyzer=analyzer, cg_options=cg_options, lang_name=lang_name, stmts=stmts: find_index(lambda y_5: k_1 == y_5, analyzer.TokenFragments), ReferencedNamedTokens, ObjectExpr168())
    ...
    with get_enumerator(lexer_maps) as enumerator:
        while enumerator.System_Collections_IEnumerator_MoveNext():
            ...
            if contains(k_2, analyzer.IgnoreSet):
                ...
                if contains_1(k_2, ReferencedNamedTokens, ObjectExpr170()):
    ...
    arr : List[str] = sort(list(analyzer.LiteralTokens), ObjectExpr171())
