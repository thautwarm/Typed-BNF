...
def codegen(analyzer: Analyzer, cg_options: CodeGenOptions, lang_name: str, stmts: List[definition]) -> List[Tuple[str, Doc]]:
    ...
    abandoned_names : Any = of_array(["and", "as", "assert", "asr", "begin", "class", "constraint", "do", "done", "downto", "else", "end", "exception", "external", "false", "for", "fun", "function", "functor", "if", "in", "include", "inherit", "initializer", "land", "lazy", "let", "lor", "lsl", "lsr", "lxor", "match", "method", "mod", "module", "mutable", "new", "nonrec", "object", "of", "open", "or", "private", "rec", "sig", "struct", "then", "to", "true", "try", "type", "val", "virtual", "when", "while", "with"], ObjectExpr149())
    class ObjectExpr150:
        @property
        def Compare(self) -> Any:
            return lambda x_5, y_1: compare(x_5, y_1)
        
    symmap : Any = empty_1(ObjectExpr150())
    ...
    def arrow_152(i: int, c: str, analyzer: Analyzer=analyzer, cg_options: CodeGenOptions=cg_options, lang_name: str=lang_name, stmts: List[definition]=stmts) -> bool:
        test : bool = True if (True if (is_lower(c)) else (is_unicode(c))) else (c == "_")
        return test if (i == 0) else (True if (test) else (is_digit(c)))
    
    ocaml_var_ident_descr : NameMangling_IdentifierDescriptor = NameMangling_IdentifierDescriptor__WithNameEnv_Z7613F24B(NameMangling_IdentifierDescriptor_Create_Z48C5CCEF(arrow_152, lambda i_1, c_1, analyzer=analyzer, cg_options=cg_options, lang_name=lang_name, stmts=stmts: to_text(interpolate("_X%P()_", [i_1])) if (is_digit(c_1)) else (c_1.lower() if (is_upper(c_1)) else (to_text(interpolate("_%P()_", [ord(c_1)]))))), NameMangling_nameEnv(of_list(map_1(lambda tuple, analyzer=analyzer, cg_options=cg_options, lang_name=lang_name, stmts=stmts: tuple[1], global_scope), ObjectExpr151())))
    class ObjectExpr153:
        @property
        def Compare(self) -> Any:
            return lambda x_7, y_3: compare_primitives(x_7, y_3)
        
    sedlex_ident_descr : NameMangling_IdentifierDescriptor = NameMangling_IdentifierDescriptor__WithNameEnv_Z7613F24B(NameMangling_IdentifierDescriptor_Create_Z48C5CCEF(lambda i_2, c_2, analyzer=analyzer, cg_options=cg_options, lang_name=lang_name, stmts=stmts: is_upper(c_2) if (i_2 == 0) else (True if (True if (is_upper(c_2)) else (c_2 == "_")) else (is_digit(c_2))), lambda i_3, c_3, analyzer=analyzer, cg_options=cg_options, lang_name=lang_name, stmts=stmts: c_3.upper() if (is_lower(c_3)) else (NameMangling_maskChar(ord("A"), ord("Z"), ord(c_3)) + "_" if (i_3 == 0) else (("_" + NameMangling_maskChar(ord("A"), ord("Z"), ord(c_3))) + "_"))), NameMangling_nameEnv(of_array(["EOF"], ObjectExpr153())))
    class ObjectExpr156:
        @property
        def Compare(self) -> Any:
    ...
    def cg_expr(scope: FSharpList[Tuple[str, str]], curr_expr: expr, analyzer: Analyzer=analyzer, cg_options: CodeGenOptions=cg_options, lang_name: str=lang_name, stmts: List[definition]=stmts) -> Doc:
        ...
        if match_value_1.tag == 6:
            ...
            def arrow_165(scope: FSharpList[Tuple[str, str]]=scope, curr_expr: expr=curr_expr) -> Iterable[Tuple[str, str]]:
                def arrow_164(match_value_3: Tuple[str, monot]) -> Iterable[Tuple[str, str]]:
                    arg : str = match_value_3[0]
                    return singleton((arg, mangle(ocaml_var_ident_descr, arg)))
                
                return collect(arrow_164, match_value_1.fields[0])
            
            code : Doc = cg_expr(append(to_list(delay(arrow_165)), scope), match_value_1.fields[1])
    ...
    def cg_stmt(stmt: definition, analyzer: Analyzer=analyzer, cg_options: CodeGenOptions=cg_options, lang_name: str=lang_name, stmts: List[definition]=stmts) -> Doc:
        ...
        if stmt.tag == 2:
            ...
            def arrow_177(stmt: definition=stmt) -> Iterable[Doc]:
                def arrow_174(match_value_4: Tuple[position, production_1]) -> Iterable[Doc]:
                    ...
                    return singleton(arrow_173())
                
                return collect(arrow_174, decl["define"])
            
            body_4 : Doc = align(vsep(map_indexed(lambda i_7, e_1, stmt=stmt: Doc_op_Addition_Z7CFFAC00(word(":") if (i_7 == 0) else (word("|")), e_1), to_list(delay(arrow_177)))))
    ...
    def arrow_204(analyzer: Analyzer=analyzer, cg_options: CodeGenOptions=cg_options, lang_name: str=lang_name, stmts: List[definition]=stmts) -> Iterable[Doc]:
            ...
            def arrow_200(_unit=None) -> Iterable[Doc]:
                ...
                def arrow_198(_unit=None) -> Iterable[Doc]:
                    def arrow_196(_unit=None) -> Iterable[Doc]:
                        def arrow_182(match_value_6: Tuple[str, Any]) -> Iterable[Doc]:
                            typename_0027 : str = type_renamer(match_value_6[0])
                            def arrow_180(_unit=None) -> Iterable[Doc]:
                                def arrow_179(match_value_7: Tuple[str, FSharpList[Tuple[str, monot]]]) -> Iterable[Doc]:
                                ...
                                return append_1(collect(arrow_179, to_array_1(match_value_6[1])), delay(lambda _unit=None: singleton(empty_2)))
                            
                            return append_1(singleton(word(to_text(interpolate("and %P() = ", [typename_0027])))), delay(arrow_180))
                        
                        def arrow_194(_unit=None) -> Iterable[Doc]:
                            def arrow_187(match_value_9: Tuple[str, Shape]) -> Iterable[Doc]:
                                ...
                                def arrow_186(_unit=None) -> Iterable[Doc]:
                                    def arrow_185(_unit=None) -> Iterable[Doc]:
                                        ...
                                        return append_1(singleton(word("}")), delay(arrow_184))
                                    
                                    return append_1(singleton(Doc_op_RightShift_2AAA0F3C(vsep(to_list(delay(lambda _unit=None: collect(lambda match_value_11: singleton(Doc_op_Addition_Z7CFFAC00(Doc_op_Addition_Z7CFFAC00(word(match_value_11[0]), word(":")), Doc_op_Multiply_Z7CFFAC00(word(match_value_11[1]), word(";")))), fields_2)))), 4)), delay(arrow_185))
                                
                                return append_1(singleton(Doc_op_Addition_Z7CFFAC00(Doc_op_Addition_Z7CFFAC00(Doc_op_Addition_Z7CFFAC00(Doc_op_Addition_Z7CFFAC00(word("and"), ret_t_1), word("=")), word("MK_" + typename_0027_1)), word("of unit"))), delay(arrow_183)) if (is_empty(fields_2)) else (append_1(singleton(Doc_op_Addition_Z7CFFAC00(Doc_op_Addition_Z7CFFAC00(Doc_op_Addition_Z7CFFAC00(word("and"), ret_t_1), word("=")), word("{"))), delay(arrow_186)))
                            
                            def arrow_193(_unit=None) -> Iterable[Doc]:
                                def arrow_192(match_value_12: Tuple[str, str, FSharpList[Tuple[str, str]], Doc]) -> Iterable[Doc]:
                                ...
                                return collect(arrow_192, doc_ctor_wrap_funcs)
                            
                            return append_1(collect(arrow_187, Sigma__GetRecordTypes(analyzer.Sigma)), delay(arrow_193))
                        
                        return append_1(collect(arrow_182, adt_cases), delay(arrow_194))
                    
                    return append_1(singleton(word("type ___used_t_head_90xasda")), delay(arrow_196))
                
                return append_1(singleton(empty_2), delay(arrow_198))
            
            return append_1(singleton(word(to_text(interpolate("open %P();;", [capitalized(filename_lexer)])))), delay(arrow_200))
        
        return append_1(singleton(word(to_text(interpolate("open %P();;", [capitalized(filename_require)])))), delay(arrow_202))
    
    file_constructors : Tuple[str, Doc] = (filename_constructors + ".ml", vsep(to_list(delay(arrow_204))))
    ...
    sort_in_place_by(lambda k_1, analyzer=analyzer, cg_options=cg_options, lang_name=lang_name, stmts=stmts: find_index(lambda y_5: k_1 == y_5, analyzer.TokenFragments), ReferencedNamedTokens, ObjectExpr205())
    ...
    arr : List[str] = sort(list(analyzer.LiteralTokens), ObjectExpr207())
    ...
    with get_enumerator(lexer_maps) as enumerator:
        while enumerator.System_Collections_IEnumerator_MoveNext():
            ...
            if contains(k_3, analyzer.IgnoreSet):
                ...
                if contains_1(k_3, ReferencedNamedTokens, ObjectExpr210()):
